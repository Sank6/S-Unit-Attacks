...
def attack(n, sgens, alphas, writer, structures, params):
...
if __name__ == '__main__':
    ...
    with open(filename, 'w', newline='') as csvfile:
        titles = ['n', 'k', 'start_index', 'end_index', 'iteration', 'num_inner_iteration','alpha_in', 'alpha_in_norm', 'alpha_out', 'alpha_out_norm', 'time']
        ...
        for n in range(min_n, max_n + 1, 1):
            ...
            for k in range(1, len(sgens), step_size):
                for start_index in range(0, len(sgens) - k):
                    end_index = start_index + k
                    select_sgens = sgens[start_index:end_index]
                    attack(n, select_sgens, alphas, writer, (K, OK, G, UK, units, embeddings), (start_index, end_index))